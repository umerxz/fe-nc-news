        ...
        deleteComment(commentId)
            .then(() => {
                setArticleComments((articleComments) =>
                    articleComments.filter((comment) => comment.comment_id !== commentId)
                );
                setErrorMsg('');
                setLoading(false);
            })
    ...
    return (
        <section className="comments-list">
            ...
            {articleComments.map((comment, index) => (
                <Card className="individualCommentBox" key={comment.comment_id ? comment.comment_id : index} variant="outlined">
                    <CardContent orientation="horizontal">
                        <Avatar src="https://via.placeholder.com/44" alt="Avatar image" />
                        <div style={{ textAlign: 'left', fontSize: '12px' }}>
                            <Typography style={{ textAlign: 'left', fontSize: '15px' }} variant="text">
                                {comment.author}
                            </Typography>
                            <Typography style={{ textAlign: 'left', fontSize: '10px' }} level="body-sm">
                                At {new Date(comment.created_at).toLocaleTimeString()} on {new Date(comment.created_at).toLocaleDateString()}
                            </Typography>
                        </div>
                    </CardContent>
                    <CardContent>
                        ...
                        <div className="commentIcons">
                            ...
                            <div className="editDelete">
                                <FontAwesomeIcon
                                    ...
                                    size="2xs"
                                />
                                <FontAwesomeIcon className={comment.author === user.username || article.author === user.username ? 'editAllow' : 'editDisallow'} icon={faPenToSquare} size="2xs" />
                            </div>
                        </div>
